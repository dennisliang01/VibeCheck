"""Hybrid Observability & Debug - Static analysis + LLM (GPT-4o Mini)."""

from typing import List
from agents.hybrid_base_agent import HybridBaseAgent, HybridConfig
from agents.observability_debug import ObservabilityDebug
from core.models import CodeFragment, AgentResult, AgentType, ValidationContext
from prompts import OBSERVABILITY_PROMPT


class HybridObservabilityDebug(HybridBaseAgent):
    """Hybrid Observability agent. Uses GPT-4o Mini for observability patterns."""
    ...
    def __init__(self, hybrid_config: HybridConfig = None):
        ...
        super().__init__(AgentType.OBSERVABILITY, config)
        self.static_agent = ObservabilityDebug()
    ...
    async def _llm_analysis(
        ...
        response = await self.llm_client.analyze(
            prompt=prompt,
            context={"task": "observability_analysis"},
            temperature=0.1,
            max_tokens=4000,
