"""Hybrid Resilience Manager - Static analysis + LLM (Claude 3.5 Sonnet)."""
...
from prompts import RESILIENCE_PROMPT


class HybridResilienceManager(HybridBaseAgent):
    """Hybrid Resilience Manager. Uses Claude 3.5 Sonnet for edge cases and failure modes."""
    ...
    def __init__(self, hybrid_config: HybridConfig = None):
        ...
        self.static_agent = ResilienceManager()
    ...
    async def _llm_analysis(
        ...
        response = await self.llm_client.analyze(
            prompt=prompt,
            context={"task": "resilience_analysis"},
            temperature=0.1,
            max_tokens=4000,
