"""Hybrid Technical Debt Hunter - Static analysis + LLM (GPT-4o Mini)."""

from typing import List
from agents.hybrid_base_agent import HybridBaseAgent, HybridConfig
from agents.technical_debt_hunter import TechnicalDebtHunter
from core.models import CodeFragment, AgentResult, AgentType, ValidationContext
from prompts import TECHNICAL_DEBT_PROMPT


class HybridTechnicalDebtHunter(HybridBaseAgent):
    """Hybrid Technical Debt Hunter. Uses GPT-4o Mini for fast code smell detection."""
    ...
    def __init__(self, hybrid_config: HybridConfig = None):
        ...
        super().__init__(AgentType.TECHNICAL_DEBT, config)
        self.static_agent = TechnicalDebtHunter()
    ...
    async def _llm_analysis(
        ...
        prompt = TECHNICAL_DEBT_PROMPT.format(
            code=code_context,
            file_path=file_path,
        ...
        response = await self.llm_client.analyze(
            prompt=prompt,
            context={"task": "technical_debt_analysis"},
            temperature=0.1,
            max_tokens=4000,
