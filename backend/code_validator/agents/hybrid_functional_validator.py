"""Hybrid Functional Validator - Static analysis + LLM (GPT-4o Mini)."""

from typing import List
from agents.hybrid_base_agent import HybridBaseAgent, HybridConfig
from agents.functional_validator import FunctionalValidator
from core.models import CodeFragment, AgentResult, AgentType, ValidationContext
from prompts import FUNCTIONAL_ANALYSIS_PROMPT


class HybridFunctionalValidator(HybridBaseAgent):
    """Hybrid Functional Validator. Uses GPT-4o Mini for cost-effective test/functional analysis."""
    ...
    def __init__(self, hybrid_config: HybridConfig = None):
        ...
        super().__init__(AgentType.FUNCTIONAL, config)
        self.static_agent = FunctionalValidator()
    ...
    async def _llm_analysis(
        ...
        prompt = FUNCTIONAL_ANALYSIS_PROMPT.format(
            code=code_context,
            file_path=file_path,
        ...
        response = await self.llm_client.analyze(
            prompt=prompt,
            context={"task": "functional_analysis"},
            temperature=0.1,
            max_tokens=4000,
